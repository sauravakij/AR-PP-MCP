...
SELECT
  COUNT(*) AS PlanLines,
  COALESCE(SUM(CASE WHEN difference = 0 THEN 1 ELSE 0 END), 0) AS OnTarget,
  COALESCE(SUM(CASE WHEN difference <> 0 THEN 1 ELSE 0 END), 0) AS VarianceFlagged,
  COALESCE(SUM(CASE WHEN (intVarianceReasonId IS NULL OR strIssueStatus IS NULL)
                    AND difference <> 0 THEN 1 ELSE 0 END), 0) AS IssuesMissing
...
WHERE v.isActive = 1
  ...
  AND (@ToDateExclusive IS NULL OR v.dteServerDateTime < @ToDateExclusive);
`;
